...
name = "Diffusion"
# blur kernel
model, diffusion = create_256_uncond()
lam =0.05
rho = 0.5
num_iters = 10
for idx, img_path in enumerate(images[:20]):
    ...
    for sigma in [0.2, 0.1, 0.01,]:
        ...
        x = deconv_admm_guided_diffusion_custom(b, None, c, lam, rho, num_iters, sigma, model, diffusion, diffusion_iters)
        x = np.clip(x, 0.0, 1.0)

        # calculating stats and writing output image
        psnr_ = psnr(img, x)
